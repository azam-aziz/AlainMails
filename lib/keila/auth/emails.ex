defmodule Keila.Auth.Emails do
  ...
  def build(:activate, %{user: user, url: url}) do
    new()
    |> from({"AlainMails", system_from_email()})
  ...
  def build(:password_reset_link, %{user: user, url: url}) do
    ...
    |> text_body(
      dgettext(
        ...
        you have requested a password reset for your AlainMails account.
  ...
  def build(:login_link, %{user: user, url: url}) do
    ...
    |> text_body(
      dgettext(
        ...
        you can login to AlainMails with the following link:
