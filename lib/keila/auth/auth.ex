defmodule Keila.Auth do
  ...
  1. Create a `User` by specifying an email address and an optional
     ...
     Provide a callback function for  generating the verification link.
        {:ok, user} = Auth.create_user(%{email: "foo@alainmails.com", password: "BatteryHorseStaple"}, url_fn: &url_fn/1)
  ...
  until `update_user_email_from_token/1` is called.
      ...
      {:ok, _token} = Auth.update_user_email(user_id, %{"email" => "new@alainmails.com"}, &url_fn/1)
  ...
  ### Example
      ...
      {:ok, alice} = Auth.create_user(%{email: "alice@alainmails.com"})
      {:ok, bob} = Auth.create_user(%{email: "alice@alainmails.com"})
  ...
  ## Example
      update_user_password(user_id, %{"email" => "new@alainmails.com"})
  """
